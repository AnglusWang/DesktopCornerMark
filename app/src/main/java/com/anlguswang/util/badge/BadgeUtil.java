...
public class BadgeUtil {
    ...
    /**
     * @param notification    更新角标一般都是和发送notification并行的。
     * @param notifyID        notification id
     * @param context
     * @param thisNotifyCount notifyID对应的notification的未读总数(XiaoMi 发送的是这个count,
     *                        XiaoMI 会自动计算所有notification 未读数量只和)
     * @param count           整个app所有的未读数量（其他的是使用这个count）
     */
    public static void sendBadgeNotification(Notification notification, int notifyID,
                                             Context context, int thisNotifyCount, int count) {
        if (count <= 0) {
            count = 0;
        } else {
